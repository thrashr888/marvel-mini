  ...
  render: function () {
    ...
    if (this.state.comics) {
      var featuredComicList = this.state.featuredComics.comics.map(function (comic, index) {
        return <Comic comic={comic} key={'feat-comics' + comic.id} className="col-lg-6 col-md-6 col-sm-12" />;
      });
      var comicList = this.state.comics.comics.map(function (comic, index) {
        return <Comic comic={comic} key={'comics' + comic.id} className="col-lg-3 col-md-4 col-sm-6" />;
      });
    } else {
    ...
    if (this.state.creators) {
      var creatorList = this.state.creators.creators.map(function (creator, index) {
        return <Creator creator={creator} key={'creators' + creator.id} className="col-lg-2 col-md-3 col-sm-4" />;
      });
    } else {
      return <div><p>Loading...</p></div>;
    }
